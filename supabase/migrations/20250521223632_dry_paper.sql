/*
  ...
  1. Creates demo user accounts in auth.users
  2. Creates corresponding profiles
  3. Sets up mock Stripe customers and subscriptions
  
  Note: Uses safe inserts that check for existing records first
*/
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.stripe_customers WHERE user_id = '00000000-0000-0000-0000-000000000001') THEN
    ...
    VALUES ('00000000-0000-0000-0000-000000000001', 'cus_demo1', now(), now());
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM public.stripe_customers WHERE user_id = '00000000-0000-0000-0000-000000000002') THEN
    INSERT INTO public.stripe_customers (user_id, customer_id, created_at, updated_at)
    VALUES ('00000000-0000-0000-0000-000000000002', 'cus_demo2', now(), now());
  END IF;
END $$;
